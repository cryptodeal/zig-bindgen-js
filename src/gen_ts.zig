...
pub const FnInfo = struct { args: []FnArgs, returns: ?FnReturns };
...
pub const TSExports = struct {
    ...
    pub fn init(import_path: []const u8, front_matter: []const u8) !*TSExports {
        ...
        try buffer.writer().print(
            ...
            \\
        , .{});
        // write import statement for addon
        try buffer.writer().print("import addon from '{s}';\n{s}", .{ import_path, front_matter });
    ...
    pub fn wrap_method(self: *TSExports, comptime name: []const u8, comptime func: anytype, comptime info: FnInfo) !void {
        ...
        inline for (info.args, 0..) |arg, i| {
            ...
            var ts_type = if (arg.mapped_type.?) arg.mapped_type else self.parse(Args.i);
            try self.buffer.writer().print("{s}: {s}", .{ arg.name, ts_type });
            if (i != info.args.len - 1) {
                try self.buffer.writer().print(", ");
            }
        }
        var res_type = if (info.returns.?) info.returns.mapped_type else self.write(Res.?, "");
